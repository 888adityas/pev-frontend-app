...
export function DashboardChart({
  ...
  const { status: downloadReportStatus, error: downloadReportError } = useSelector(
    (state) => state.emailList.downloadReport
  );
  ...
  const handleOpen = (mode) => {
    ...
    setSelectedOption('all'); // Reset selection when opening dialog
  };
  ...
  const handleDownload = () => {
    if (!currRow.bulk_verify_id) {
      console.log("Job ID not found, can't download report!");
      return;
    }
    // downloadReportStatus, downloadReportError
    try {
      setIsDownloading(true);
      dispatch(downloadReport({ jobId: currRow.bulk_verify_id, filter: selectedOption }));
      console.log(' Report will download in few seconds.');
      setTimeout(() => {
  ...
  // For async notifications [download report]
  useMemo(() => {
    if (downloadReportStatus === 'success') {
      toast.success('Download is in progress. Please wait...');
      dispatch(clearDownloadReport());
    } else if (downloadReportStatus === 'failed') {
      ...
      <Scrollbar>
        <Card {...other}>
          ...
          >
            ...
            {currRow && currRow.status === 'verified' && (
          ...
          {showProgressLinear && <ProgessLinear setIsDrawerOpen={setIsDrawerOpen} />}
        </Card>
      </Scrollbar>
      <Alert
        sx={{
